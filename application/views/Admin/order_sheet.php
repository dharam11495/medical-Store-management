<?php include('header.php'); ?>
   ...
   <div class="container">
     <div class="row">
      <div class="col-md-12 text-center" style="margin-bottom: 50px;"><h4>Order Balance Sheet</h4></div><br>
...
<!-- ajax/medicine_fetc -->
    <script>
        ...
        function showAllEmployee(){
            $.ajax({
                type: 'ajax',
                url: '<?php echo base_url() ?>ajax/medicine_fetch_order',
                async: false,
                dataType: 'json',
                success: function(data){
                    ...
                    for(i=0; i<data.length; i++){
                      var s=data[i].pending;
                      var u=data[i].quantity;
                     
                      p='';
                      t='';
                      if (s==u) {
                        s='<p style="color:#32CD32;">Ordered</p>'
                      
                       
                      }else{
                        s='<p style="color:#FF4500;">Pending Some Ordered</p>'
                        ...
                        '<td style="border: 1px solid black">'+'<a href="javascript:;" class="btn btn-info item-details" style="background-color: #808000" data="'+data[i].id+'">Details</a>'+'</td>'+
                                    ...
                                    '<td style="border: 1px solid black">'+data[i].salt_name+'</td>'+
                                     ...
                                     '<td style="border: 1px solid black">'+'<a href="javascript:;" class="btn btn-info item-edit" style="background-color: #abd680" data="'+data[i].id+'">Take In</a>'+'</td>'+                                    
    ...
    $('#myTable').on('click', '.item-edit', function(){
      ...
      $.ajax({
        ...
        error: function(){
          alert('Could not Take In dfghdfgdfgdf');
        }
      });
    });
 ...
 // Update Data
    ...
    $('#btnSave').click(function(){
      ...
      if(parseInt(take_in_quantity.val())>parseInt(quantity.val())){
          alert('You Can not take in extra');
          exit;
      }else{
        result +='6';
      }
    ...
    $('#myTable').on('click', '.item-details', function(){
      ...
      $.ajax({
        type: 'ajax',
        method: 'get',
        url: '<?php echo base_url() ?>ajax/ordertrack',
        data: {id: id},
        async: false,
        dataType: 'json',
        success: function(data){
          ...
          <form id="myForm" action="" method="post">
            <input type="hidden" name="txtId" >
             ...
             <input type="text" class="form-control" name="variats_name" placeholder="Enter Trade Name" readonly>
              ...
              <div class="form-group row">
              <label for="inputPassword" class="col-sm-4 col-form-label">Order Quantity</label>
              <div class="col-sm-8">
              <input type="text" class="form-control" name="order" readonly>
              </div>
              </div>
               <div class="form-group row">
              ...
              <input type="hidden" class="form-control" name="stock" >
  ...
  <div class="modal-dialog modal-lg" role="document">
    ...
    <tbody >
      ...
      <tr style="border: 1px solid black">
       ...
       <tr style="border: 1px solid black">
        <td style="border: 1px solid black;  color: #663399;">Order(Pending) Quantity</td>
